...
_current_input_handler: Optional[Callable[[str], str]] = None


def set_output_handler(handler: Optional[Callable[[str, str], None]]):
    """Sets the output handler for the current execution."""
    global _current_output_handler
    _current_output_handler = handler


def get_output_handler() -> Optional[Callable[[str, str], None]]:
    """Gets the current output handler."""
    global _current_output_handler
    return _current_output_handler


def set_input_handler(handler: Optional[Callable[[str], str]]):
    """Sets the input handler for the current execution."""
    global _current_input_handler
    _current_input_handler = handler


def get_input_handler() -> Optional[Callable[[str], str]]:
...
class UnaryOperation(BaseExpression):
    __operations = {"+": operator.pos, "-": operator.neg, "~": operator.inv, "not": operator.not_}
...
class PrintStatement(BaseExpression):
    ...
    def eval(self):
        global _current_output_handler  # Access the global handler
        # Evaluate items and convert them to strings
        evaluated_items = self.items.eval()
        # Join items without spaces, as original print did end='', sep=''
        output_string = "".join(map(str, evaluated_items))

        if _current_output_handler:
            _current_output_handler(output_string, "stdout")  # Use handler
        else:
            print(output_string, end="", sep="")  # Original behavior
